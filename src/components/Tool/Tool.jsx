    ...
    return (
        <div className='max-w-300 m-auto mb-10 bg-amber-100 py-12 space-y-4'>
            ...
            <div className="tabs flex justify-center bg-gray-50 w-43 m-auto rounded-full p-0 shadow">
                <input type="radio" name="my_tabs_1" className="btn rounded-full" aria-label="Products" defaultChecked />

                <input type="radio" name="my_tabs_1" className="btn rounded-full" aria-label={`Cart (${0})`} />
            </div>


            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 my-5'>
                {
                    toolsData.map(toolObj=> <Products key={toolObj.id} toolObj={toolObj}></Products>)
                }
            </div>
        </div>
    );
